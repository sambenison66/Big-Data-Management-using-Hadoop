...
public class AvgTempNOAAYearly {
  ...
  // Mapper Class for AvgTempNOAA-Yearly
  public static class MapReduceMapper extends MapReduceBase
    ...
    public void map(LongWritable key, Text val,
      ...
      if(StringUtils.isNumeric(rawDate)) {
	      String dateKey = rawDate.substring(0, 4); // Took Year from Date
  ...
  // Reducer class for AvgTempNOAA-Yearly
  public static class MapReduceReducer extends MapReduceBase
    ...
    public void reduce(Text key, Iterator<Text> values,
        ...
        throws IOException {
           ...
           if (arrayTempCount > 1) {
        	   int compInt = arrayTemp.get(arrayTempCount - 2);
        	   if(compInt < avgTemp) {
           ...
           if (arrayPrcpCount > 1) {
        	   int compInt = arrayPrcp.get(arrayPrcpCount - 2);
        	   if(compInt < avgPrcp) {
  ...
  // Main Driver class for AvgTempNOAA-Yearly
  
  public static void main(String[] args) {

    JobConf conf = new JobConf(AvgTempNOAAYearly.class);
    conf.setJobName("AvgTempNOAAYearly");
